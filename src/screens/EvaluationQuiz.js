...
function EvaluationQuizPage({ navigation, route }) {
  const { quiz, userData } = route.params;
  ...
  const title =
    userData.category === "Student"
      ? userData.ageGroup + " Evaluation Test"
      : "Employee Category Evaluation Test";
  const handleDone = () => {
    if (selectedAnswers.includes(null)) {
      alert("Please answer all questions before submitting");
    } else {
      navigation.navigate("Evaluation Test Results", {
        title,
        quiz,
        selectedAnswers,
        hintsViewed,
